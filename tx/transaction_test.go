...
func TestTx(t *testing.T) {
	to, _ := .ParseAddress("0x7567d83b7b8d80addcb281a71d54fc7b3364ffed")
...
func TestIntrinsicGas(t *testing.T) {
	...
	gas, err = tx.IntrinsicGas(tx.NewClause(&.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, .TxGas+.ClauseGas, gas)
	...
	gas, err = tx.IntrinsicGas(tx.NewClause(&.Address{}), tx.NewClause(&.Address{}))
	assert.Nil(t, err)
...
func BenchmarkTxMining(b *testing.B) {
	...
	signer := .BytesToAddress([]byte("acc1"))
	maxWork := &big.Int{}
