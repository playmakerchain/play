...
func TestTx(t *testing.T) {
	to, _ := powerplay.ParseAddress("0x7567d83b7b8d80addcb281a71d54fc7b3364ffed")
	...
	assert.Equal(t, powerplay.Bytes32{}, trx.ID())
...
func TestIntrinsicGas(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, powerplay.TxGas+powerplay.ClauseGas, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(&powerplay.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, powerplay.TxGas+powerplay.ClauseGas, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(nil))
	assert.Nil(t, err)
	assert.Equal(t, powerplay.TxGas+powerplay.ClauseGasContractCreation, gas)

	gas, err = tx.IntrinsicGas(tx.NewClause(&powerplay.Address{}), tx.NewClause(&powerplay.Address{}))
	assert.Nil(t, err)
	assert.Equal(t, powerplay.TxGas+powerplay.ClauseGas*2, gas)
}

func BenchmarkTxMining(b *testing.B) {
	tx := new(tx.Builder).Build()
	signer := powerplay.BytesToAddress([]byte("acc1"))
	maxWork := &big.Int{}
