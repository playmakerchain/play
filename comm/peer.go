...
import (
	...
	"github.com/playmakerchain/powerplay/p2psrv/rpc"
...
func (p *Peer) Head() (id powerplay.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
...
func (p *Peer) UpdateHead(id powerplay.Bytes32, totalScore uint64) {
	p.head.Lock()
	defer p.head.Unlock()
