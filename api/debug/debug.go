...
import (
	...
	"github.com/playmakerchain/play/consensus"
	"github.com/playmakerchain/play/play"
	"github.com/playmakerchain/play/runtime"
	"github.com/playmakerchain/play/state"
	"github.com/playmakerchain/play/tracers"
	"github.com/playmakerchain/play/trie"
	"github.com/playmakerchain/play/vm"
)

var devNetGenesisID = play.MustParseBytes32("0x00000000973ceb7f343a58b08f0693d6701a5fd354ff73d7058af3fba222aea4")
...
func (d *Debug) handleTxEnv(ctx context.Context, blockID play.Bytes32, txIndex uint64, clauseIndex uint64) (*runtime.Runtime, *runtime.TransactionExecutor, error) {
...
func (d *Debug) traceTransaction(ctx context.Context, tracer vm.Tracer, blockID play.Bytes32, txIndex uint64, clauseIndex uint64) (interface{}, error) {
...
func (d *Debug) debugStorage(ctx context.Context, contractAddress play.Address, blockID play.Bytes32, txIndex uint64, clauseIndex uint64, keyStart []byte, maxResult int) (*StorageRangeResult, error) {
...
func storageRangeAt(t *trie.SecureTrie, start []byte, maxResult int) (*StorageRangeResult, error) {
	...
	for i := 0; i < maxResult && it.Next(); i++ {
		...
		v := play.BytesToBytes32(content)
		e := StorageEntry{Value: &v}
		if preimage := t.GetKey(it.Key); preimage != nil {
			preimage := play.BytesToBytes32(preimage)
			e.Key = &preimage
		}
		result.Storage[play.BytesToBytes32(it.Key).String()] = e
	}
	if it.Next() {
		next := play.BytesToBytes32(it.Key)
		result.NextKey = &next
	}
	return &result, nil
}
...
func (d *Debug) parseTarget(target string) (blockID play.Bytes32, txIndex uint64, clauseIndex uint64, err error) {
	parts := strings.Split(target, "/")
	if len(parts) != 3 {
		return play.Bytes32{}, 0, 0, utils.BadRequest(errors.New("target:" + target + " unsupported"))
	}
	blockID, err = play.ParseBytes32(parts[0])
	if err != nil {
		return play.Bytes32{}, 0, 0, utils.BadRequest(errors.WithMessage(err, "target[0]"))
	}
	if len(parts[1]) == 64 || len(parts[1]) == 66 {
		txID, err := play.ParseBytes32(parts[1])
		if err != nil {
			return play.Bytes32{}, 0, 0, utils.BadRequest(errors.WithMessage(err, "target[1]"))
		}
		txMeta, err := d.chain.GetTransactionMeta(txID, blockID)
		if err != nil {
			if d.chain.IsNotFound(err) {
				return play.Bytes32{}, 0, 0, utils.Forbidden(errors.New("transaction not found"))
			}
			return play.Bytes32{}, 0, 0, err
		}
	...
	if err != nil {
		return play.Bytes32{}, 0, 0, utils.BadRequest(errors.WithMessage(err, "target[2]"))
	}
	return
}
