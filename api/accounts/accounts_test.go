...
import (
	...
	ABI "github.com/playmakerchain/play/abi"
	"github.com/playmakerchain/play/api/accounts"
...
var addr = play.BytesToAddress([]byte("to"))
var value = big.NewInt(10000)
var storageKey = play.Bytes32{}
...
func getStorage(t *testing.T) {
	...
	h, err := play.ParseBytes32(value["value"])
	if err != nil {
		t.Fatal(err)
	}
	assert.Equal(t, play.BytesToBytes32([]byte{storageValue}), h, "storage should be equal")
	assert.Equal(t, http.StatusOK, statusCode, "OK")
}
...
func initAccountServer(t *testing.T) {
	...
	contractAddr = play.CreateContractAddress(transaction.ID(), 1, 0)
