...
// Account is the PowerPlay consensus representation of an account.
...
func (a *Account) CalcEnergy(blockTime uint64) *big.Int {
	...
	x.Mul(x, a.Balance)
	x.Mul(x, powerplay.EnergyGrowthRate)
	x.Div(x, bigE18)
...
func loadAccount(trie trieReader, addr powerplay.Address) (*Account, error) {
...
func saveAccount(trie trieWriter, addr powerplay.Address, a *Account) error {
	if a.IsEmpty() {
...
func loadStorage(trie trieReader, key powerplay.Bytes32) (rlp.RawValue, error) {
	return trie.TryGet(key[:])
}

// saveStorage save value for given key.
// If the data is zero, the given key will be deleted.
func saveStorage(trie trieWriter, key powerplay.Bytes32, data rlp.RawValue) error {
	if len(data) == 0 {
