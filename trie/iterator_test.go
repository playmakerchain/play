...
func TestNodeIteratorCoverage(t *testing.T) {
	...
	hashes := make(map[powerplay.Bytes32]struct{})
	for it := trie.NodeIterator(nil); it.Next(true); {
		if it.Hash() != (powerplay.Bytes32{}) {
	...
	for _, key := range db.(*ethdb.MemDatabase).Keys() {
		if _, ok := hashes[powerplay.BytesToBytes32(key)]; !ok {
...
func TestIteratorContinueAfterSeekError(t *testing.T) {
	...
	db := ethdb.NewMemDatabase()
	ctr, _ := New(powerplay.Bytes32{}, db)
	for _, val := range testdata1 {
		ctr.Update([]byte(val.k), []byte(val.v))
	}
	root, _ := ctr.Commit()
	barNodeHash, _ := powerplay.ParseBytes32("d32fb77ad25227d60b76d53a512d28137304c9c03556db08a1709563c7ae9c9f")
