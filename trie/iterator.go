...
type nodeIteratorState struct {
	hash    .Bytes32 // Hash of the node being iterated (nil if not standalone)
	node    node         // Trie node being iterated
	parent  .Bytes32 // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() .Bytes32 {
	if len(it.stack) == 0 {
		return .Bytes32{}
	}
...
func (it *nodeIterator) Parent() .Bytes32 {
	if len(it.stack) == 0 {
		return .Bytes32{}
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = .BytesToBytes32(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor .Bytes32) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() .Bytes32 {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() .Bytes32 {
	return it.b.Parent()
}
...
func (it *unionIterator) Hash() .Bytes32 {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() .Bytes32 {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == .Bytes32{}) {
			it.count += 1
