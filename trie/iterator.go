...
type NodeIterator interface {
	...
	Hash() powerplay.Bytes32
	// Parent returns the hash of the parent of the current node. The hash may be the one
	// grandparent if the immediate parent is an internal node with no hash.
	Parent() powerplay.Bytes32
...
type nodeIteratorState struct {
	hash    powerplay.Bytes32 // Hash of the node being iterated (nil if not standalone)
	node    node         // Trie node being iterated
	parent  powerplay.Bytes32 // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() powerplay.Bytes32 {
	if len(it.stack) == 0 {
...
func (it *nodeIterator) Parent() powerplay.Bytes32 {
	if len(it.stack) == 0 {
		return powerplay.Bytes32{}
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = powerplay.BytesToBytes32(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor powerplay.Bytes32) (*nodeIteratorState, []byte, bool) {
		...
		for i := parent.index + 1; i < len(node.Children); i++ {
			...
			if child != nil {
				...
				state := &nodeIteratorState{
					hash:    powerplay.BytesToBytes32(hash),
					node:    child,
					parent:  ancestor,
					index:   -1,
					pathlen: len(it.path),
...
func (it *differenceIterator) Hash() powerplay.Bytes32 {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() powerplay.Bytes32 {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == powerplay.Bytes32{}
...
func (it *unionIterator) Hash() powerplay.Bytes32 {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() powerplay.Bytes32 {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == powerplay.Bytes32{}) {
			it.count += 1
